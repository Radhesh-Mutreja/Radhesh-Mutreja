...
```bash
radhesh@nfsu-delhi:~/profile$ cat whoami.txt
```

```
Name     : Radhesh Mutreja
Role     : MSc DFIS Student & Security Researcher
Uni      : National Forensic Sciences University, Delhi
...
```bash
radhesh@nfsu-delhi:~/profile$ cat current_ops.txt
```
...
```bash
radhesh@nfsu-delhi:~/profile$ ls ./skills/
```
...
```bash
radhesh@nfsu-delhi:~/profile$ cat connect.txt
```
